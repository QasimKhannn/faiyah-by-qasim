...
export const Navigation: React.FC<Props> = ({ toggle }: Props) => {
    ...
    return (
        <motion.ul
            variants={variants}
            className="flex flex-col md:flex-row md:justify-between items-center justify-center w-full h-full"
        >
            <div className="flex md:justify-start items-start justify-center flex-col md:px-20 gap-4">
                {menuItems.map((i) => (
                    <MenuItem
                        i={i.id}
                        key={i.id}
